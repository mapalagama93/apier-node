...
class Initializer {
    sample_request = `name: Sample request
...
`
    files = ['/configs/config.properties', '/configs/vars.properties', '/scripts/functions.js', '/sample.yaml']
    dirs = ['/configs', '/scripts']
    file_content = {
            '/sample.yaml' : this.sample_request,
            '/scripts/functions.js' : "import vars",
            '/configs/config.properties' : "scripts_dir=scripts"
    }

    init() {
        this.files = this.files.concat(args.env.map((e) => '/configs/' + e + '.properties'));
        this.createDirs();
        this.createFiles();
    }

    createDirs(){
        for(let x of this.dirs) {
          let dir = args.root + x;
